...
public class TerrainGenerator : MonoBehaviour {
	...
	// Prototypes
	public Texture2D m_detail0;
	public Texture2D m_splat0;
	public float m_splatTileSize0 = 3.0f;
	public GameObject m_tree0;
	...
	public int m_heightMapSize = 257; // Higher number will create more detailed height maps
	public int m_alphaMapSize = 256; // This is the control map that controls how the splat textures will be blended
	public int m_terrainSize = 60;
	public int m_terrainHeight = 30;
	public int m_detailMapSize = 256; // Resolutions of detail (Grass) layers
	//Tree settings
	...
	void Start () {
		m_groundSeed = (int)Random.value * 1000;
		m_mountainSeed = (int)Random.value * 1000;
		m_treeSeed = (int)Random.value * 1000;
		m_detailSeed = (int)Random.value * 1000;
		...
		m_offset = new Vector2(-10, -30);
		
		CreateProtoTypes();
		
		FillHeights(htmap, 1, 1);
		...
		GameObject temp = Terrain.CreateTerrainGameObject(terrainData);
		temp.transform.parent = this.transform;
		m_terrain = temp.GetComponent<Terrain>();
		m_terrain.transform.position = new Vector3(m_terrainSize*0 + m_offset.x, 0, m_terrainSize*0 + m_offset.y);
		...
		FillTreeInstances(m_terrain, 0, 0);
		FillDetailMap(m_terrain, 1, 1);
		
	}

	void CreateProtoTypes () {
		m_splatPrototypes = new SplatPrototype[1];
		...
		m_treeProtoTypes = new TreePrototype[1];
		
		m_treeProtoTypes[0] = new TreePrototype();
		m_treeProtoTypes[0].prefab = m_tree0;
		
		m_detailProtoTypes = new DetailPrototype[1];
		...
		m_detailProtoTypes [0].maxHeight = 1;
		m_detailProtoTypes [0].minHeight = .5f;
		m_detailProtoTypes [0].maxWidth = 1;
		m_detailProtoTypes [0].minWidth = .5f;
	}
	...
	{
		float[,,] map  = new float[m_alphaMapSize, m_alphaMapSize, 1];
